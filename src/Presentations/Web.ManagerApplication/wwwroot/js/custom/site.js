            ...
            success: function (data) {
                ...
                $("#supplereinvoice").select2({
                    ...
                    if (e.params.data != null) {
                        ...
                        $(".btn-publishEinvoice").data("typesignature", e.params.data.type)
                    } else {
                        $(".btn-sendCQT").data("typesignature", -1);
                        $(".btn-publishEinvoice").data("typesignature", -1);
                    }
        ...
        $(".btn-publishEinvoice").click(function () {
            ...
            if (parseInt(checktypesign) == ENumTypeSeri.TOKEN) {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Thông báo',
                    html: "Chưa hỗ trợ ký số SmartCA",
                    showCloseButton: true,
                    showCancelButton: true,
                    cancelButtonText: '<i class="fa fa-window-close"></i> Đã hiểu',
                });
                ...
                loadeventEinvoice.publishinvoice(lstid);
            }
...
var eventInvocie = {
    loadActionShowHideColumn: function () {
        $(".btn-invoiceshow").click(function () {
            let active = false;
            ...
            html = `
                ...
                <div class="index-z"></div>
                    ...
                    <li>
                     ...
                     <li>
                        <input type="checkbox" class="icheck" value="12"/>
                        Giảm giá
                   </li>
                     <li>
                        <input type="checkbox" class="icheck" value="13"/>
                        Tổng cộng
                     </li>
                     <li>
                        <input type="checkbox" class="icheck" value="14"/>
                        Trạng thái
                    <li>
                    </ul>
            </div>`;
            $(this).after(html);
            var getcolumnloca = localStorage.getItem("columninvoiceshow");
          //  var lstcolumn = [0,1,4,10,11,12,13,14,15];
            var lstcolumn = [2, 3, 5, 6, 7, 8, 9];
            if (typeof getcolumnloca != "undefined" && getcolumnloca != null) {
                lstcolumn = JSON.parse(getcolumnloca);
            } else {
                localStorage.setItem("columninvoiceshow", JSON.stringify(lstcolumn));
            }
            ...
            loadEventIcheck();
            eventInvocie.eventSaveColumnsTable();
            $(document).mouseup(function (e) {
                var container = $(".ullistitemcomlumsCustome");
                // if the target of the click isn't the container nor a descendant of the container
                if (!container.is(e.target) && container.has(e.target).length === 0) {
                    $(".list-comlumsCustome").remove();
                    $(".btn-invoiceshow").removeClass("active");
                }
            });
    ...
    eventSaveColumnsTable: function () {
        $(".list-comlumsCustome input[type='checkbox']").on('ifChanged', function (event) {
            var getcolumnloca = JSON.parse(localStorage.getItem("columninvoiceshow"));
            let value = parseInt($(this).val());
            if ($(this).prop("checked") == true) {
            ...
            localStorage.setItem("columninvoiceshow", JSON.stringify(getcolumnloca));
    ...
    gethashTokenPublishEinvoice: async function (lstid,typeSupplierEInvoice, serialCert, serial, pattern, xml) {
        loadingStart();
        dataObject = {};
        dataObject.type = TypeEventWebSocket.SignListEInvoiceToken;
       ...
       await sposvietplugin.sendConnectSocket(listport[0]).then(async function () {
           ...
           await  sposvietplugin.connectSignatureWebSocket(listport[0], JSON.stringify(dataObject)).then(function (data) {
               if (data == "-1" || data == "-2") {
                   eventInvocie.deletePublisEinvoiceByTokenError(lstid);
                   loadingStop();
               }
               else { 
    ...
    deletePublisEinvoiceByTokenError: function (lstid) {
        $.ajax({
            type: 'POST',
            //global: false,
            url: '/Selling/Invoice/DeleteEInvoiceErrorInPublish',
            data: {
                lstid: lstid,
            },
            success: function (res) {
    ...
    },//xóa hoa đơn điện tử nếu người dùng hủy bỏ
...
var posStaff = {
    ...
    loadDanhSachBan: function () {
        ...
        $(".topbuton button:first").removeAttr("disabled");
    },//load các bàn có ra
...
var loadeventPos = {
    ...
    getTotalServiceByTime: function (num, price) {
        ...
        let quantity = _rhours + parseFloat((_rminutes / 60).toFixed(2));
    ...
    eventUpdateTotalProductByService: async function () {
        ...
        $("#item-mon li.isServiceDate").map(function () {
            if ($(this).data("datecreateservice") != null && $(this).data("datecreateservice") != "") {//nếu sản phẩm có tính giờ
                ...
                //tính tiền và số lượng
                let _hours = (timeDiff / 60);
                let _rhours = Math.floor(_hours);
                let _minutes = (_hours - _rhours) * 60;
                let _rminutes = Math.round(_minutes);
                let quantity = _rhours + parseFloat((_rminutes / 60).toFixed(2));

                let mony = Math.round(quantity * price);//lấy tổng tiền theo phút trên
    ...
    eventShowInfoFoodService: function () {
        ...
        $("#item-mon .btnServicedate").click(function () {
           ...
           // let date = moment($(this).data("datecreateservice")).format("HH:mm");
            ...
            let _rminutes = Math.round(_minutes);
            let quantity = _rhours + parseFloat((_rminutes / 60).toFixed(2));
            let mony = Math.round(quantity * price);
            //kiểm tra nếu khác với dữ liệu hiện tại thì update luôn, do interval chạy chưa tới
            if (parseFloat($(this).parents("li.itemorder").find(".quantity").val()) != quantity) {
                loadeventPos.eventUpdateTotalProductByService().then(function (data) {
                    if (data) {//nếu có dịch vụ thì tính tiền lại
                        loadeventPos.updateTotalIsFoodByService();
                    }
            ...
            $('.fc-datetimepicker').on('apply.daterangepicker', function (ev, picker) {
                ...
                if ($(this).hasClass("startdate")) {//dành cho cập nhật thời gian bắt đầu tính
                    ...
                    start = true;//tức là điều chỉnh cho input start, là ngày bắt đầu
                } else {
                    ...
                    if ($(".tableinfo-foodisservice .btn-actiontime").hasClass("stop")) {//chỉ cho update khi đang dừng tính giờ
                        ...
                        toastrcus.error("Thời gian kết thúc không được chỉnh sửa khi vẫn đang tính giờ");
                        let oldvalue = $(".enddate").data("value");
                        loadeventPos.updateEndDateFoodservice($(".enddate"), oldvalue);
                        return false;
                    }
                    
                    start = false;//ngược lại là kết thúc
                }
    ...
    eventhandelinvoice: function () {
        $(".handelinvoice").click(function () {
            ...
            loadeventPos.eventShowHideColumns();
    ...
    eventShowHideColumns: function () {
        $(".btn-invoiceposshow").click(function () {
            ...
            loadEventIcheck();
            loadeventPos.eventSaveColumnsTable();
            $(document).mouseup(function (e) {
                //var container = $(".list-comlumsCustome");
                var container = $(".ullistitemcomlumsCustome");
                // if the target of the click isn't the container nor a descendant of the container
    ...
    loadEventClickIconAddAndMinus: function () {
        ...
        $("#item-mon  li .item_action").find("i:first-child").click(function () {
            ...
            if ($(this).parents("li.itemorder").hasClass("isServiceDate")) {
                toastr.error("Hàng hóa là sản phẩm dịch vụ tính tiền theo giờ, không thể điều chỉnh số lượng");
                return;
            }
            //bấm giảm thì phải hỏi
                    ...
                    if (res.isValid) {
                        Swal.fire({
                            ...
                            didRender: () => {
                                ...
                                $(".btn-save").click(function () {
                                    let pub = true;
                                    if ($("#ManagerPatternEInvoices").length > 0) {
                                        let getpatern = $("#ManagerPatternEInvoices option:selected").text();
                                        let serial = getpatern.split('-')[1].trim();
                                        if (serial.substring(0, 1) == "C" && serial.substring(3, 4) == "M") {
                                            pub = true;
                                        } else {
                                            toastrcus.error("Hệ thống chỉ hỗ trợ phát hành hóa đơn điện tử máy tính tiền, vui lòng chọn lại đúng mẫu số, ký hiệu hóa đơn!");
                                            pub = false;
                                        }
                                    }
                                    if (pub) {
