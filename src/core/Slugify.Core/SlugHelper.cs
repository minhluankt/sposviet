...
namespace Slugify
{
    public class SlugHelper : ISlugHelper
    {
        ...
        {
            inputString = this.ToSlug(inputString.ToLower());
        ...
        private void ApplyStringReplacements(StringBuilder sb)
        {
            ...
            {
                var search = replacement.Key.Normalize(NormalizationForm.FormKD);
                var replace = replacement.Value.Normalize(NormalizationForm.FormKD);
        ...
        protected string ToSlug(string s)
        {
            if (s == null)
            {
                return s;
