    ...
    public class CategorysProductController : BaseController<CategorysProductController>
    {
        ...
        public async Task<IActionResult> Getjsontreeview()
        {
           ...
           // var currentUser = await _userManager.GetUserAsync(HttpContext.User);
            var response = await _mediator.Send(new GetAllCategoryProductCacheQuery() { IsPos=true,Comid= currentUser.ComId});
        ...
        public async Task<ActionResult> OnPostCreateOrEdit(CategoryViewModel model)
        {
            ...
            var currentUser = User.Identity.GetUserClaimLogin();
           // var currentUser = await _userManager.GetUserAsync(HttpContext.User);
            var collection = new CategoryProduct { Icon = model.Icon, Name = model.fullName,
                ComId = currentUser.ComId,
                IsPos = true,
                Sort = model.Sort, Id = model.id, IdPattern = model.IdPattern, Active = model.active = true };
            try
            {
                if (collection.Id == 0)
                {
